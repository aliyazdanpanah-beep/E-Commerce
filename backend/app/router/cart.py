...
class CartItemRequest(BaseModel):
    ...
    quantity: int = Field(gt=0, default=1, description="Quantity of product")
...
async def add_to_cart(
    ...
    if existing_item:
        ...
        cart_item = CartItem(
            ...
            price_at_add=float(product.price)  # Store current price
        )
        db.add(cart_item)
        db.commit()
...
async def update_cart_item(
    ...
    cart = db.query(Cart).filter(Cart.id == cart_item.cart_id, Cart.user_id == user.get('id')).first()
    if cart is None:
